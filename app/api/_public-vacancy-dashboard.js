...
export function buildAnonymousVacancyDashboard(publicPayload, vacancySync, generatedAt) {
  const jobs = publicPayload.jobs;
  const percent = (count) => jobs.length ? Math.round(count / jobs.length * 100) : 0;
