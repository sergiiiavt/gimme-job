...
export type VacancyActivityEnv = {
  DB?: D1Database;
...
function authorizationError(request: Request, env: VacancyActivityEnv): Response | null {
  const configured = env.N8N_INGEST_TOKEN?.trim() ?? "";
  if (!configured) return json({ error: "Vacancy automation activity is not configured." }, 503);
...
export async function handleVacancyAutomationActivity(request: Request, env: VacancyActivityEnv): Promise<Response> {
