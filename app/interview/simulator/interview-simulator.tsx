...
export default function InterviewSimulator() {
  ...
  const strongAreas = useMemo(() => {
    ...
    return [...byCategory.entries()]
      .map(([category, scores]) => ({ category, score: Math.round(scores.reduce((sum, value) => sum + value, 0) / scores.length) }))
        ...
        <div className={`kb-content ${styles.page}`}>
          <header className={styles.hero}>
            <div>
              ...
              <p className={styles.lead}>Answer real questions from the GimmeJob catalog. AI evaluates the answer against the maintained reference material and remembers your weak areas.</p>
            </div>
            ...
            <div className={styles.setupGrid}>
              <section className={styles.panel}>
                <div className={styles.panelHeader}>
                  <div>
                    <p className={styles.kicker}>New session</p>
                  ...
                  <fieldset className={styles.fieldset}>
                    ...
                    <div className={styles.segmented}>
                      {countOptions.map((count) => (
                        <button className={questionCount === count ? styles.selected : ""} key={count} onClick={() => setQuestionCount(count)} type="button">{count}</button>
                      ))}
                    </div>
                  </fieldset>
                ...
                <button className={styles.primaryButton} disabled={busy || authRequired} onClick={start} type="button">
              ...
              <section className={styles.panel}>
                <div className={styles.panelHeader}>
                  <div>
                    <p className={styles.kicker}>Progress memory</p>
                    ...
                    {progress.areas.slice(0, 6).map((area) => (
                      <div className={styles.areaRow} key={`${area.track}-${area.category}`}>
                        <div>
                          ...
                          <span>{readableTrack(area.track)} · {area.attempts} answer{area.attempts === 1 ? "" : "s"}</span>
                        </div>
          ...
          {stage === "question" && currentQuestion && (
            <section className={styles.interviewLayout}>
              <div className={styles.sessionTopline}>
                <div>
                  <span>{readableTrack(currentQuestion.track)}</span>
              ...
              <article className={styles.questionCard}>
                <div className={styles.questionMeta}>
                  <span>{currentQuestion.prevalence}</span>
                ...
                {!evaluation ? (
                  <>
                    <label className={styles.answerField}>
                      ...
                      <textarea
                        ...
                        placeholder="Answer as you would in a real interview. A concise answer is fine if it covers the key points."
                        rows={9}
                        value={answer}
                      />
                    </label>
                    <div className={styles.answerActions}>
                      ...
                      <button className={styles.primaryButton} disabled={busy || !answer.trim()} onClick={submit} type="button">
                  ...
                  <div className={styles.evaluation}>
                    <div className={styles.scoreHeader}>
                      ...
                      <div>
                        <p className={styles.rating}>{evaluation.rating}</p>
                    ...
                    <div className={styles.feedbackGrid}>
                      <div className={styles.feedbackCard}>
                        ...
                        {evaluation.strengths.length ? <ul>{evaluation.strengths.map((item) => <li key={item}>{item}</li>)}</ul> : <p>No clear strength identified yet.</p>}
                      </div>
                      <div className={styles.feedbackCard}>
                        <h3>What to improve</h3>
                        {evaluation.gaps.length ? <ul>{evaluation.gaps.map((item) => <li key={item}>{item}</li>)}</ul> : <p>No material gap identified.</p>}
                      </div>
                    </div>
                    ...
                    <details className={styles.referenceAnswer}>
                      ...
                      {evaluation.strong_answer_signals.length > 0 && (
                        <div className={styles.signalList}>
                          {evaluation.strong_answer_signals.map((signal) => <span key={signal}>{signal}</span>)}
                        </div>
                      )}
                    </details>
                    ...
                    {evaluation.follow_up_question && (
                      <div className={styles.followUp}>
                        <span>Likely follow-up</span>
                        <strong>{evaluation.follow_up_question}</strong>
                      </div>
                    )}
                    ...
                    {evaluation.recommended_topics.length > 0 && (
                      <div className={styles.topicList}>
                        ...
                        <div>{evaluation.recommended_topics.map((topic) => <span key={topic}>{topic}</span>)}</div>
                      </div>
                    )}

                    <div className={styles.nextRow}>
                      <span>Answer saved to your interview progress.</span>
                      <button className={styles.primaryButton} onClick={() => void next()} type="button">
          ...
          {stage === "complete" && (
            <section className={styles.completeCard}>
              ...
              <div className={styles.completeHeadline}>
                <div className={`${styles.finalScore} ${scoreClass(averageScore ?? 0)}`}>
                  <strong>{averageScore ?? 0}</strong>
                  <span>/100 average</span>
                </div>
                <div>
                  <h2>{readableTrack(track)} interview result</h2>
                  <p>{results.length} answers evaluated. These results are now part of your progress history.</p>
                </div>
              </div>
              ...
              <div className={styles.summaryGrid}>
                <div>
                  ...
                  {strongAreas.filter((item) => item.score >= 65).slice(0, 4).map((item) => (
                    <div className={styles.summaryRow} key={item.category}><span>{item.category}</span><strong>{item.score}%</strong></div>
                  ))}
                  {!strongAreas.some((item) => item.score >= 65) && <p className={styles.muted}>No area reached 65% in this session.</p>}
                </div>
                <div>
                  <h3>Focus next</h3>
                  {strongAreas.filter((item) => item.score < 65).slice(0, 4).map((item) => (
                  ...
                  {!strongAreas.some((item) => item.score < 65) && <p className={styles.muted}>No weak category in this session.</p>}
                </div>
              </div>
