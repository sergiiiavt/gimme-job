...
export default function VacanciesPage() {
  ...
  useEffect(() => {
    if (!authenticated && statusFilter !== "ALL") setStatusFilter("ALL");
    if (!authenticated && selectedIds.size) setSelectedIds(new Set());
  }, [authenticated, selectedIds.size, statusFilter]);

  const visibleJobs = useMemo(() => jobs
    .filter((job) => !authenticated || statusFilter === "ALL" || job.status === statusFilter)
    .filter((job) => displayText(`${job.title} ${job.company} ${job.location} ${job.source}`).toLowerCase().includes(query.toLowerCase()))
    .sort((a, b) => jobDate(b).getTime() - jobDate(a).getTime()), [authenticated, jobs, query, statusFilter]);
  ...
  const mode = authenticated ? "personal" : "public";

  return (
    <main className="kb-shell">
      <SiteSidebar
        activeSection="jobs"
        activeSubsection={authenticated ? statusFilter : "ALL"}
        hideSecondary
        mobileOpen={mobileNav}
        mode={mode}
        onSelectSubsection={(next) => { if (authenticated) setStatusFilter(next as JobStatus | "ALL"); setMobileNav(false); }}
      ...
      <section className="kb-main kb-main-compact-nav">
        ...
        <div className={`vacancies-page vacancies-page-${mode}`}>
          <header className="vacancies-header">
            ...
            <div className={`vacancies-stats ${authenticated ? "vacancies-stats-personal" : "vacancies-stats-public"}`}>
              {authenticated ? <>
                <Stat value={personalCounts.total} label="Total"/>
                <Stat value={personalCounts.new} label="New"/>
                <Stat value={personalCounts.applied} label="Applied"/>
                <Stat value={personalCounts.interviews} label="Interviews"/>
              </> : <>
                <Stat value={publicCounts.total} label="Total"/>
                <Stat value={publicCounts.remote} label="Remote"/>
                <Stat value={publicCounts.reservation} label="Reservation"/>
              </>}
            </div>
          ...
          {selected ? <section className={`job-detail-view ${authenticated ? "job-detail-view-personal" : "job-detail-view-public"}`} id="selected-vacancy-detail" role="region" aria-label="Selected vacancy details">
            ...
            {authenticated && <article className="job-analysis-resume">
          ...
          </section> : <section className="vacancy-list-panel">
            <div className={`vacancy-tools ${authenticated ? "vacancy-tools-personal" : "vacancy-tools-public"}`}>
              ...
              {authenticated && <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as JobStatus | "ALL")} aria-label="Filter by status">
                <option value="ALL">All statuses</option>
                {STATUS_OPTIONS.map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}
              </select>}
            </div>

            <div className="vacancy-feed-meta">
            ...
            <div className={`vacancy-column-head vacancy-column-head-${mode}`} aria-hidden="true">
              {authenticated && <span/>}
              <span>Vacancy</span>
              {authenticated && <span>Match</span>}
              <span>Posted</span>
              {authenticated && <span>Status</span>}
            </div>

            <div className="vacancy-feed">
              {visibleJobs.map((job) => <div
                key={job.id}
                className={`vacancy-row vacancy-row-${mode}`}
                role="button"
                tabIndex={0}
              ...
              >
                {authenticated && <input
                  type="checkbox"
                  className="job-card-check vacancy-check"
                ...
                <div className="vacancy-main">
                  <strong>{displayText(job.title)}</strong>
                  <p>{displayText(job.company)} · {displayText(job.location)}</p>
                  <div className="chips">
                    <span>{sourceLabel(job.source)}</span>
                    {job.remote && <span>Remote</span>}
                    {job.salaryText && <span>{displayText(job.salaryText)}</span>}
                    {hasReservation(job) && <span className="reservation">Бронювання</span>}
                    {authenticated && job.feedback === "RELEVANT" && <span className="good">Relevant</span>}
                    {authenticated && job.feedback === "NOT_RELEVANT" && <span className="bad">Not relevant</span>}
                  </div>
                </div>

                {authenticated && <div className={`vacancy-match ${typeof job.analysis?.score === "number" ? "has-score" : "no-score"}`}>
                  <strong>{typeof job.analysis?.score === "number" ? job.analysis.score : "—"}</strong>
                  <span>{typeof job.analysis?.score === "number" ? verdictLabel(job.analysis.verdict) : "Not analyzed"}</span>
                </div>}

                <time className="vacancy-date" dateTime={job.postedAt ?? job.discoveredAt} title={formatDate(job.postedAt ?? job.discoveredAt)}>{formatCompactDate(job.postedAt ?? job.discoveredAt)}</time>

                {authenticated && <span className={`status status-${job.status.toLowerCase().replace("_", "-")}`}>{statusLabel(job.status)}</span>}
              </div>)}
