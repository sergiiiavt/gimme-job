...
4. Authenticate the `gimmejob_user_session` at the Worker boundary and inject a trusted `user_id`; then enable multi-user sign-in for a controlled beta.
5. Add tenant-safe analysis and resume generation. Until then those write operations fail closed in multi-user mode rather than touching legacy global tables.
6. Add Gmail metadata synchronization into `user_email_events`.
7. Add Gmail push notifications through Google Cloud Pub/Sub and renew `users.watch` daily (Gmail watches expire and must be renewed at least every 7 days).
8. Add AI classification, job matching, and approval-first reply drafting.
