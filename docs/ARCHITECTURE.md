...
GimmeJob has **one retrieval system**. `worker/rag.ts` owns the canonical corpus and retrieval policy for learning material, QA/Python interview questions, and jobs. Git and D1 remain authoritative data sources. Cloudflare Workers AI produces multilingual BGE-M3 embeddings and Vectorize provides semantic candidate ranking when their bindings are available. Lexical ranking is a resilience mode inside the same canonical pipeline, not a second RAG implementation. Every consumer receives the same retrieval strategy/result contract.
