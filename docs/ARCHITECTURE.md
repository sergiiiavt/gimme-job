...
Git-backed learning discovery is integrated through `content/learning-rag-registry.ts`. A learning surface is registered there once; `worker/rag.ts` derives topic/lesson documents and first-class Markdown `##` section documents from the registered content, including canonical `topic`, `section`, and `track` navigation metadata. The Python AI service and Learning Advisor browser validate canonical learning route shape instead of maintaining duplicated route allow-lists. Normal deployment already performs the post-deploy Vectorize refresh. The detailed contract and extension rules are documented in `docs/LEARNING-RAG.md`.
...
The production Hetzner VM is managed by the files under `ops/hetzner/`. n8n, its PostgreSQL database, and the Python AI service use private Docker networking; Caddy publishes HTTP/HTTPS and proxies `n8n.gimme-job.com` to n8n and `ai.gimme-job.com` to `gimmejob-ai:8000`. The MySQL and PostgreSQL database labs have no published host ports; provider firewall rules also block direct access. The browser calls `/api/playgrounds/databases`; the Worker authenticates with a server-only service token to Caddy at `ai.gimme-job.com/db-lab/*`, which forwards to the internal `db-lab-api:8080` sidecar. That sidecar is the bridge to the separate lab Docker network and uses restricted SQL users for queries. Its database clients disable client-side shell commands. Named volumes preserve data across normal deployments and restarts. Sessions map to four shared practice workspaces per engine; only an explicit reset recreates a workspace. Their synthetic fixture volumes are separate from the private n8n database and GimmeJob application state.

Hetzner runtime changes are deployed from `main`: provisioning reconciles the VM, firewall, and DNS, while the runtime-refresh workflow uses Hetzner rescue access to install the current version-controlled bootstrap on the VM, reboot into Ubuntu, recreate the Compose runtime, and verify n8n, database-lab health, closed direct database ports, and rejection of API requests without a service token. The trusted `ai-image.yml` workflow separately builds and publishes the AI image to GHCR. On `main` pushes or a manual dispatch, its deployment job copies the protected AI runtime environment to the VM over SSH, pulls the public GHCR image, starts the Compose `ai` profile, reloads Caddy, and verifies `/health`. Pull-request runs build the image only and never receive production deployment secrets. The initial AI activation requires the deployment SSH key and AI/Langfuse secrets to be configured in GitHub. The Worker uses `GIMMEJOB_AI_URL=https://ai.gimme-job.com` and its independent service token to reach the service.

## Security boundaries

- no production secrets in Git;
- production data remains in private D1/private service databases;
- the private MySQL/PostgreSQL labs contain synthetic practice data only and are isolated from production state;
- database-lab credentials and the service token are never sent to the browser;
