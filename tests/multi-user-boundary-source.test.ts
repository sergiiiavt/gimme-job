...
test("routing helpers accept only internal workspace return paths", () => {
  ...
  assert.equal(googleLoginLocation("/workspace"), "/auth/google/start?next=%2Fworkspace");
...
test("identity headers are always removed before the legacy core sees a request", async () => {
  ...
  assert.equal(calls[0]!.request.headers.get("x-normal-header"), "keep-me");
});

test("multi-user login redirects anonymous users to Google and rejects unsupported methods", async () => {
  const { worker: core, calls } = fakeCore();
  const boundary = createMultiUserBoundary(core);
  const env: FakeEnv = { MULTI_USER_ENABLED: "true", DB: fakeSessionDb().db };

  const response = await boundary.fetch(
    new Request("https://example.com/workspace/login?next=%2Fworkspace%2Flearn%3Fsection%3Dinterview"),
    env,
    undefined,
  );
  assert.equal(response.status, 303);
  assert.equal(response.headers.get("location"), "/auth/google/start?next=%2Fworkspace%2Flearn%3Fsection%3Dinterview");
  assert.equal(calls.length, 0);

  const method = await boundary.fetch(new Request("https://example.com/workspace/login", { method: "DELETE" }), env, undefined);
  assert.equal(method.status, 405);
  assert.equal(method.headers.get("allow"), "GET, HEAD, POST");
});
...
test("private routes reject spoofed tenant identity without a session", async () => {
  ...
  const api = await boundary.fetch(new Request("https://example.com/api/interview-progress", {
    headers: {
      "x-gimmejob-auth-mode": "multi-user",
      "x-gimmejob-authenticated": "1",
      "x-gimmejob-user-id": "victim",
  ...
  const workspace = await boundary.fetch(new Request("https://example.com/workspace/learn?section=interview", {
    headers: { "x-gimmejob-user-id": "victim" },
  }), env, undefined);
...
test("a D1 session replaces attacker headers with trusted tenant identity", async () => {
  ...
  const env: FakeEnv = { MULTI_USER_ENABLED: "true", DB: db, APP_PASSWORD: "legacy-secret" };

  const response = await boundary.fetch(new Request("https://example.com/api/interview-progress", {
    headers: {
      cookie: "gimmejob_user_session=valid-session",
      authorization: "Basic attacker-value",
      "x-gimmejob-auth-mode": "legacy",
      "x-gimmejob-authenticated": "1",
  ...
  assert.equal(calls.length, 1);
  const call = calls[0]!;
  assert.equal(call.request.headers.get("x-gimmejob-auth-mode"), "multi-user");
  assert.equal(call.request.headers.get("x-gimmejob-authenticated"), "1");
  assert.equal(call.request.headers.get("x-gimmejob-user-id"), "user-a");
  assert.notEqual(call.env.APP_PASSWORD, "legacy-secret");
  const authorization = call.request.headers.get("authorization") ?? "";
  assert.match(authorization, /^Basic /);
  const decoded = atob(authorization.slice("Basic ".length));
  assert.equal(decoded, `gimmejob:${call.env.APP_PASSWORD}`);
});

test("public multi-user requests reach the core without private identity or Authorization", async () => {
  const { worker: core, calls } = fakeCore();
  const boundary = createMultiUserBoundary(core);
  const env: FakeEnv = { MULTI_USER_ENABLED: "true", DB: fakeSessionDb().db };

  const response = await boundary.fetch(new Request("https://example.com/", {
    headers: { authorization: "Basic client-value", "x-gimmejob-user-id": "victim" },
  }), env, undefined);
  assert.equal(response.status, 200);
  assert.equal(calls.length, 1);
  assert.equal(calls[0]!.request.headers.get("x-gimmejob-auth-mode"), "multi-user");
...
test("observability bypasses tenant auth and preserves its Bearer header", async () => {
  const { worker: core, calls } = fakeCore();
  const boundary = createMultiUserBoundary(core);
  const env: FakeEnv = { MULTI_USER_ENABLED: "true", DB: fakeSessionDb().db };
  const response = await boundary.fetch(new Request("https://example.com/api/observability/health", {
    headers: { authorization: "Bearer grafana-token", "x-gimmejob-user-id": "ignored" },
  }), env, undefined);
  assert.equal(response.status, 200);
...
test("valid multi-user login returns directly and logout is POST-only", async () => {
  ...
  const login = await boundary.fetch(new Request("https://example.com/workspace/login?next=%2Fworkspace%2Flearn", { headers }), env, undefined);
