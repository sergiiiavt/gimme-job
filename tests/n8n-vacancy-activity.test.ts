...
test("daily automation activity reports vacancy mutations separately from email classification changes", async () => {
  ...
  assert.match(queries[1]?.sql ?? "", /WHEN 'INTERVIEW' THEN 2/);
});
...
test("daily report workflow fetches and renders detailed automation activity", async () => {
  ...
  assert.match(formatNode?.parameters?.jsCode ?? "", /Email changes made by automation/);
  assert.match(formatNode?.parameters?.jsCode ?? "", /does not mean vacancy state was changed/);
  assert.ok(Object.prototype.hasOwnProperty.call(workflow.connections, "Get detailed automation activity"));
});
