...
test("ships Fight AI slop as a lazy, local-only public game", async () => {
  ...
  assert.match(gameSource, /interface TerrainCell \{ surface: SurfaceKind; corruption: CorruptionLevel; seed: number; source: number \| null \}/);
  assert.match(gameSource, /type CorruptionLevel = 0 \| 1 \| 2 \| 3 \| 4/);
  assert.match(gameSource, /function createFacilityFootprint/);
  assert.match(gameSource, /function facilityStage/);
  assert.match(gameSource, /buildProgress: 0, footprint, outlet/);
  assert.match(gameSource, /state\.tiles\[cell\.row\]\[cell\.col\]\.surface = "rubble"/);
  assert.match(gameSource, /drawPondState/);
  ...
  assert.match(gameSource, /state\.selected === "rootreclaimer" \? cell\.corruption > 0 : cell\.surface === "grass" && cell\.corruption === 0/);
