...
test("database playground is present in navigation and uses the real proxy", async () => {
  ...
  assert.match(client, /type QueryTab/);
  assert.match(client, /TABS_KEY = "gimmejob-db-lab-tabs-v1"/);
  assert.match(client, /ACTIVE_TAB_KEY = "gimmejob-db-lab-active-tab-v1"/);
  assert.match(client, /localStorage\.setItem\(TABS_KEY, JSON\.stringify\(tabs\)\)/);
  assert.match(client, /result: QueryResponse \| null/);
  assert.match(client, /engine: Engine/);
  assert.match(client, /MAX_QUERY_TABS = 12/);
  assert.match(client, /New query tab/);
  assert.match(client, /closeTab/);
  assert.match(client, /loadExample\(entry: DatabaseExample\)/);
  assert.match(client, /addTab\(engine, entry\.sql, entry\.title\)/);

  assert.match(client, /Tables & data/);
  assert.match(client, /Keys & constraints/);
  assert.match(client, /Documents & arrays/);
  assert.match(client, /Engine differences/);
  assert.match(client, /Advanced SQL/);
  assert.match(client, /Advanced pipelines/);
  assert.match(client, /CTE \+ window ranking/);
  assert.match(client, /ROW_NUMBER\(\) OVER/);
  assert.match(client, /Session variable/);
  assert.match(client, /SET @min_total = 250/);
  assert.match(client, /Conditional aggregation/);
  assert.match(client, /Correlated EXISTS/);
  assert.match(client, /CREATE OR REPLACE VIEW paid_order_summary/);
  assert.match(client, /WITH RECURSIVE sequence/);
  assert.match(client, /\$lookup/);
  assert.match(client, /\$facet/);
  assert.match(client, /\$let variables/);
  assert.match(client, /\$cond/);
  ...
  assert.match(client, /db\.qa_notes\.insertOne/);
  assert.match(client, /db\.qa_notes\.deleteOne/);
  assert.match(client, /Use example/);
  assert.match(client, /CREATE TABLE IF NOT EXISTS qa_notes/);
  assert.match(client, /DROP TABLE IF EXISTS qa_notes/);
  assert.match(client, /FOREIGN KEY \(user_id\) REFERENCES users\(id\)/);
  assert.match(client, /CREATE UNIQUE INDEX uq_qa_notes_title/);
  assert.match(client, /CREATE INDEX/);
  assert.match(client, /EXPLAIN/);

  assert.match(client, /defaultExpandedTables/);
  assert.match(client, /engine === "mongodb" \? \[\] : tables\.map/);
  assert.match(client, /expandedTables\?\.includes\(table\.name\)/);
  assert.match(client, /expanded\.includes\(name\) \? expanded\.filter/);
  assert.match(client, /aria-expanded=\{expanded\}/);
  assert.match(client, />Expand all</);
  assert.match(client, />Collapse all</);
  assert.match(client, /setExpandedTables\(schema\.map/);
  assert.match(client, /setExpandedTables\(\[\]\)/);
  assert.match(client, /Select first 100 rows from/);
  assert.match(client, /firstRowsQuery/);
  assert.match(client, /LIMIT 100/);
  assert.match(client, /db\.\$\{tableName\}\.find\(\{\}\)\.limit\(100\)/);
  assert.match(client, /executeStatement\(tabId, engine, statement\)/);

  assert.match(client, /styles\.engineSwitch/);
  assert.doesNotMatch(client, /styles\.enginePanel/);
  assert.match(client, /styles\.leftRail/);
  assert.match(client, /styles\.technicalPanel/);
  assert.match(client, /styles\.workbench/);
  assert.match(client, /styles\.queryTabs/);
  assert.match(client, /styles\.editorBody/);
  assert.match(client, /styles\.editorActions/);
  assert.match(client, /result\.rows\.map/);
  assert.match(client, /result\.documents/);
  assert.doesNotMatch(client, />Database type</);
  ...
  assert.match(styles, /grid-template-columns: 380px minmax\(0, 1fr\)/);
  assert.match(styles, /\.engineSwitch \{/);
  assert.match(styles, /\.databaseTools/);
  assert.match(styles, /\.quickSelect/);
  assert.match(styles, /\.queryTabs/);
  assert.match(styles, /\.queryTab/);
  assert.match(styles, /grid-template-rows: auto minmax\(0, 1fr\) minmax\(0, 1fr\)/);
  assert.match(styles, /\.editorBody[\s\S]*position: relative/);
  assert.match(styles, /\.editorActions[\s\S]*position: absolute/);
  assert.match(styles, /\.editorActions[\s\S]*bottom: 10px/);
  assert.match(styles, /\.leftRail/);
  assert.match(styles, /\.exampleCategories/);
  assert.match(styles, /\.mongoDocuments/);
  assert.match(styles, /overflow-wrap: anywhere/);
  assert.match(styles, /word-break: break-word/);
  assert.doesNotMatch(styles, /\.enginePanel/);
  assert.doesNotMatch(styles, /\.editorFooter/);
