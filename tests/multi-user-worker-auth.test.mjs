...
test("multi-user Worker ignores spoofed identity headers and redirects to Google login", async () => {
  ...
  assert.equal(
    ...
    "/auth/google/start?next=%2Fworkspace%2Flearn%3Fsection%3Dinterview",
  );

  const spoofedApi = await worker.fetch(
    new Request("https://gimmejob.example/api/interview-progress", {
      headers: {
        "x-gimmejob-auth-mode": "multi-user",
        "x-gimmejob-authenticated": "1",
        "x-gimmejob-user-id": "victim-user",
      },
    }),
    env,
    context,
  ...
  const spoofedWorkspace = await worker.fetch(
    new Request("https://gimmejob.example/workspace/learn?section=interview", {
      headers: {
        "x-gimmejob-auth-mode": "multi-user",
        "x-gimmejob-authenticated": "1",
        "x-gimmejob-user-id": "victim-user",
  ...
  assert.equal(
    spoofedWorkspace.headers.get("location"),
