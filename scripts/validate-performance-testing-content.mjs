...
for (const question of questions) {
  ...
  for (const sourceId of question.sourceIds) assert.ok(sourceById.has(sourceId), `Unknown performance source ${sourceId} in ${question.id}`);
}
...
assert.equal(learning.chapters.length, 8, "The base performance curriculum must retain its eight reviewed methodical chapters.");
assert.ok(learning.sources.length >= 10, "Performance learning path must use a broad primary-source set.");
assert.ok(learning.sources.every((source) => source.kind.startsWith("Official")), "Base learning-path sources must be primary official documentation or an official syllabus.");
...
for (const chapter of learning.chapters) {
  ...
  for (const sourceId of chapter.sourceIds) assert.ok(learningSourceIds.has(sourceId), `Unknown learning source ${sourceId} in chapter ${chapter.id}`);
}

assert.equal(localizationUk.chapters.length, 9, "Ukrainian metadata must cover all eight methodical chapters plus the GimmeJob Locust walkthrough.");
assert.equal(localizationUk.chapters.at(-1)?.id, "locust-performance-testing");
const ukChapterDocuments = await Promise.all(
  Array.from({ length: 8 }, (_, index) => readJson(`../content/performance-testing/chapter-${String(index + 1).padStart(2, "0")}.uk.json`)),
);
assert.deepEqual(ukChapterDocuments.map((chapter) => chapter.id), learning.chapters.map((chapter) => chapter.id));
for (const chapter of ukChapterDocuments) assert.ok(chapter.markdownUk?.trim().length >= 1500, `Ukrainian performance chapter is too shallow: ${chapter.id}`);

for (const markdown of [locustEn.markdown, locustUk.markdownUk]) {
  assert.ok(markdown?.trim().length >= 5000, "The GimmeJob Locust walkthrough must remain detailed in both languages.");
  for (const marker of [
    "tests/performance/gimmejob/locustfile.py",
    "GET /api/health",
    "GET /api/public/jobs",
    "GET /api/dashboard",
    "between(2, 5)",
...
console.log(`Performance testing content validated: ${questions.length} researched interview questions, ${learning.chapters.length} base chapters + 1 GimmeJob Locust walkthrough, Ukrainian learning translation present.`);
