...
function optionalEnvironment(name) { return process.env[name]?.trim() || ""; }
function enabledEnvironment(name) { return optionalEnvironment(name).toLowerCase() === "true"; }
function deploymentWarning(message) { console.warn(`::warning::${message}`); }
...
function listDatabases() { return parseJsonList(runWrangler(["d1", "list", "--json"], { capture: true }), "D1 database"); }
function findDatabase(databases) { return databases.find((database) => database?.name === databaseName) ?? null; }
function databaseId(database) {
  const value = database?.uuid ?? database?.id;
  return typeof value === "string" && value.length > 0 ? value : null;
}
function listVectorIndexes() { return parseJsonList(runWrangler(["vectorize", "list", "--json"], { capture: true }), "Vectorize index"); }
function vectorIndexExists(indexes) { return indexes.some((index) => index?.name === vectorIndexName); }
...
function aiServiceConfiguration() {
  ...
  if (Boolean(url) !== Boolean(serviceToken)) throw new Error("GIMMEJOB_AI_URL and GIMMEJOB_AI_SERVICE_TOKEN must be configured together.");
  if (!url) return { configured: false, url: "", serviceToken: "" };
  let parsed;
  try { parsed = new URL(url); }
  catch { throw new Error("GIMMEJOB_AI_URL must be a valid URL."); }
...
async function writeDeploymentSecrets({ appPassword, grafanaReadToken, n8nIngestToken, mcpServiceToken, ragServiceToken, googleAuth, aiService }) {
  const secrets = {
    APP_PASSWORD: appPassword,
    GRAFANA_READ_TOKEN: grafanaReadToken,
    N8N_INGEST_TOKEN: n8nIngestToken,
    MCP_SERVICE_TOKEN: mcpServiceToken,
    ...(ragServiceToken ? { GIMMEJOB_RAG_SERVICE_TOKEN: ragServiceToken } : {}),
  };
  ...
  for (let attempt = 1; attempt <= 8; attempt += 1) {
    try {
      const response = await fetch(url, {
        method: "POST",
        headers: { "content-type": "application/json", "x-gimmejob-mcp-token": token },
...
async function validateConfig() {
  ...
  try {
    ...
    await writeFile(deploySecretsPath, `${JSON.stringify({
      ...
      GIMMEJOB_RAG_SERVICE_TOKEN: "dry-run-rag-service-token",
    })}\n`, { mode: 0o600 });
    runWrangler(["deploy", "--dry-run", "--config", generatedConfigPath, "--secrets-file", deploySecretsPath]);
  } finally {
    await Promise.all([rm(generatedConfigPath, { force: true }), rm(deploySecretsPath, { force: true })]);
  }
}

async function main() {
  await mkdir(path.join(wranglerRuntimePath, "xdg-config"), { recursive: true });
  if (process.argv.includes("--dry-run")) { await validateConfig(); return; }
  ...
  const ragServiceToken = optionalEnvironment("GIMMEJOB_RAG_SERVICE_TOKEN");
  ...
  if (ragServiceToken && ragServiceToken.length < 32) throw new Error("GIMMEJOB_RAG_SERVICE_TOKEN must contain at least 32 characters when configured.");
  if (aiService.configured && !ragServiceToken) throw new Error("GIMMEJOB_RAG_SERVICE_TOKEN is required when GimmeJob AI is configured.");
  ...
  try {
    ...
    await writeDeploymentSecrets({ appPassword, grafanaReadToken, n8nIngestToken, mcpServiceToken, ragServiceToken, googleAuth, aiService });
